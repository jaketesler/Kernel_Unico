...
#endif
/******************* LVDS *******************/
#define MX6_SECO_UQ7_LVDS_BLT_CTRL  IMX_GPIO_NR(1, 6)
/******************* HDMI *******************/
#define MX6_SECO_UQ7_HDMI_CHECK		IMX_GPIO_NR(5, 2)
/******************* ETHERNET *******************/
...
#endif
/******************* CAN *******************/
#define MX6_SECO_UQ7_CAN1_STBY		IMX_GPIO_NR(1, 2)
#define MX6_SECO_UQ7_CAN1_EN		IMX_GPIO_NR(1, 4)

#define MX6_SECO_UQ7_CAP_TCH_INT1	IMX_GPIO_NR(1, 9)
#define MX6_SECO_UQ7_CSI0_RST		IMX_GPIO_NR(4, 22)
#define MX6_SECO_UQ7_CSI0_PWN		IMX_GPIO_NR(4, 21)

#define MX6_SECO_UQ7_IO_MICRO       IMX_GPIO_NR(2, 4)
...
static void tsc2006_dev_init(void) {
	...
	if (gpio_request(MX6_SECO_UQ7_HDMI_CHECK, "TSC2006 pendown") < 0) {
		printk("can't get tsc2006 pen down GPIO\n");
	}

        gpio_direction_input(MX6_SECO_UQ7_HDMI_CHECK);
	msleep(5);
}
...
static struct spi_board_info imx6_seco_q7_spi_device[] __initdata = {
#if defined(CONFIG_Q7_MTD_M25P80)
	{
		...
		.irq 			= gpio_to_irq(MX6_SECO_UQ7_HDMI_CHECK),
        },
#endif
};
...
static int tsc2004_get_pendown_state (void) {
        return gpio_get_value(MX6_SECO_UQ7_HDMI_CHECK);
}
...
static void __init mx6_seco_q7_board_init(void)
{
	...
	ret = gpio_request (MX6_SECO_UQ7_IO_MICRO, "IO MICRO");
	if (ret) {
		printk("failed to get MX6_SECO_UQ7_IO_MICRO: %d\n", ret);
	} else {	
		gpio_direction_input (MX6_SECO_UQ7_IO_MICRO);
	}
...
#endif
#if defined(CONFIG_UQ7_SD1_INTERFACE) // SD/MMc external Card on CROSSPLATFORM
